...
async function main() {
  const filePath = path.join(__dirname, "secados.csv");
  if (!fs.existsSync(filePath)) {
    console.error("Crear prisma/secados.csv con columnas: caravana,fecha");
    process.exit(1);
  }
  ...
  for (const line of rows) {
    ...
    const { caravana, fecha } = row;
    if (!caravana || !fecha) continue;
    ...
    inserted++;
    console.log(`  ${caravana} ${fecha} ok`);
  }
  console.log(`\nListo: ${inserted} secados importados.`);
}
